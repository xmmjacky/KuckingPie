...
namespace DTcms.Web.UI.Page
{
    public partial class index : Web.UI.BasePage
    {
        ...
        protected string taodiandianaccount;
        protected string areabusy = "0";
        protected string arealock = "0";
        protected string distributionArea = string.Empty , distributionArea_2 = string.Empty;
        ...
        protected override void ShowPage()
        {
            ...
            if (dtParentArea.Rows.Count > 0)
            {
                ...
                foreach (DataRow item in dtArea.Rows)
                {
                    ...
                    distributionArea += item["Id"].ToString() + "-" + item["DistributionArea"].ToString() + "_";
                    if (string.IsNullOrEmpty(item["DistributionArea_2"].ToString())) continue;
                    distributionArea_2 += item["Id"].ToString() + "-" + item["DistributionArea_2"].ToString() + "_";

                }
                distributionArea = distributionArea.TrimEnd('_');
                distributionArea_2 = distributionArea_2.TrimEnd('_');
            ...
            taodiandianaccount = System.Web.Security.FormsAuthentication.HashPasswordForStoringInConfigFile(config.TaoDianDianAccount, "MD5").ToLower();
            if (this.Context.Request.Cookies["AreaId_20151030"] != null)
            {
                BookingFood.Model.bf_area areaModel = bllArea.GetModel(int.Parse(this.Context.Request.Cookies["AreaId_20151030"].Value));
                if (areaModel != null)
                {
