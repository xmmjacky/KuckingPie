...
namespace DTcms.API.Payment.MpMicroPay
{
    public class MicroPay
    {
        ...
        * @return 刷卡支付结果
        */
        public static string Run(string orderno, string auth_code, string total_fee, string area_title)
        {
            Common.Log.Info("MicroPay Micropay is processing...");

            WxPayData data = new WxPayData();
            data.SetValue("auth_code", auth_code);//授权码
            data.SetValue("body", "LUCKING'PIE 馍王-" + area_title + "-条码支付");//商品描述
            ...
            if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
		    {
                ...
                Common.Log.Info("MicroPay Micropay API interface call failure, result : " + result.ToXml());
                //throw new WxPayException("Micropay API interface call failure, return_msg : " + returnMsg);
		    }

		    //签名验证
            result.CheckSign();
            Common.Log.Info("MicroPay Micropay response check sign success");

            //刷卡支付直接成功
            if(result.GetValue("return_code").ToString() == "SUCCESS" &&
		        result.GetValue("result_code").ToString() == "SUCCESS")
            {
                Common.Log.Info("MicroPay Micropay business success, result : " + result.ToXml());
                return "支付成功";
            }
		    ...
		    {
                Common.Log.Info("MicroPay micropay API interface call success, business failure, result : " + result.ToXml());
                return result.IsSet("err_code_des") ? result.GetValue("err_code_des").ToString() : "";
            }
		    ...
		    while(queryTimes-- > 0)
		    {
                ...
                {
                    Common.Log.Info("MicroPay Micropay failure, return micropay result : " + result.ToXml());
                    return result.IsSet("err_code_des") ? result.GetValue("err_code_des").ToString() : "";
                }
		    }

            //确认失败，则撤销订单
            Common.Log.Info("MicroPay Micropay failure, Reverse order is processing...");
		    //if(!Cancel(out_trade_no))
		    //{
                //Common.Log("MicroPay", "Reverse order failure");
