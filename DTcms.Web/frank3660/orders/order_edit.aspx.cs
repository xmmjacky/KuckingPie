...
namespace DTcms.Web.admin.orders
{
    public partial class order_edit : Web.UI.ManagePage
    {
        ...
        protected string GetSubGoods(string subgoodsid)
        {
            string rtn = "";
            string[] subgoods = subgoodsid.Split('†');
            foreach (var sub in subgoods)
            {
                if (sub.Split('‡')[0] == "taste")
                {
                    rtn += string.Format("<tr><td>*{0}</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"
                        , sub.Split('‡')[1]);
                }
                else
                {
                    rtn += string.Format("<tr><td>*{0}</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>", sub.Split('‡')[2]);
                }
