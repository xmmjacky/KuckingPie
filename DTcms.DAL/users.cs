...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        public int Add(Model.users model)
        {
            ...
            strSql.Append("group_id,user_name,password,email,nick_name,avatar,sex,birthday,telphone,mobile,qq,address,safe_question,safe_answer,amount,point,exp,is_lock,reg_time,reg_ip,company_id)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@reg_ip", SqlDbType.NVarChar,30),
            new SqlParameter("@company_id", SqlDbType.TinyInt,1)};
            ...
            parameters[20].Value = model.company_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 * from dt_users ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["company_id"] != null && ds.Tables[0].Rows[0]["company_id"].ToString() != "")
                {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.users GetModel(string email,string phone)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 * from dt_users ");
            strSql.Append(" where telphone=@telphone and email != '' ");
