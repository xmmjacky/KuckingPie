...
namespace TeeGonSdk.Util
{
    ...
    public sealed class RequestValidator
    {
        ...
        public static void ValidateRequired(string name, object value)
        {
            if (value == null)
            {
                throw new TopException(string.Format(ERR_MSG_PARAM_MISSING, name));
            }
            else
            {
        ...
        public static void ValidateMaxLength(string name, string value, int maxLength)
        {
            ...
            {
                throw new TopException(string.Format(ERR_MSG_PARAM_INVALID, name));
            }
        }
        ...
        /// <param name="value">参数值</param>
        /// <param name="minValue">最小值</param>
        public static void ValidateDecimalMinValue(string name, Nullable<decimal> value, decimal minValue)
        {
            if (value != null && value < minValue)
            {
                throw new TopException(string.Format(ERR_MSG_PARAM_INVALID, name));
