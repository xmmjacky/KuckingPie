...
namespace DTcms.Common
{
    public class Utils
    {
        ...
        {
            string num = DateTime.Now.ToString("HHmmssms");//MMddHHmmssms
            return num + Number(4).ToString();
        }

        private static int OrderFix = 0;

        public static string GetOrderNumberForMp()
        {
            string num = DateTime.Now.ToString("HHmm");//MMddHHmmssms
            num = num + Number(3).ToString();
            if(OrderFix>9)
            {
                OrderFix = 0;
            }
        ...
        public static string DownloadImg(string url)
        {
            ...
            string newFileName = string.Empty; //随机生成新的文件名
            newFileName = DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".jpg"; //随机生成新的文件名
            FileStream file = null;
            file = new FileStream(fullUpLoadPath + newFileName, FileMode.Create);
        ...
        public static string DownloadImg(string url,string upLoadPath,string filename)
        {
            MemoryStream stream = new MemoryStream();
            WebClient wc = new WebClient();
            try
