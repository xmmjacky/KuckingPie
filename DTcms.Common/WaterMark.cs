...
namespace DTcms.Common
{
    public class WaterMark
    {
        ...
        public static Bitmap GetThumbnail(string imgUrl, int destHeight, int destWidth)
        {
            Bitmap b = new Bitmap(imgUrl);
        ...
        public static void AddImageToPic(string imgPath, string filename, string watermarkFilename, int watermarkStatus, int quality, int watermarkTransparency)
        {
            if(!File.Exists(Utils.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(Utils.GetMapPath(imgPath));
            ...
            //设置高质量,低速度呈现平滑程度
            //g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighQuality;
            Image watermark = GetThumbnail(watermarkFilename, 300, 300);
            ...
            switch (watermarkStatus)
            {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    xpos = (int)((img.Width * (float).53) - (watermark.Width / 2));
                    ypos = (int)((img.Height * (float).84) - watermark.Height);
                    break;
                case 9:
                    xpos = (int)((img.Width * (float).99) - (watermark.Width));
                    ypos = (int)((img.Height * (float).99) - watermark.Height);
